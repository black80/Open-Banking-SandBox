...
The application has micro-service architecture and uses Spring Boot 2.1.1. Every independent feature "xyz" maps to a top level package (`de.OFIN.psd2.sandbox.xyz`) (backend) or a top level folder (`src/app/xyz`) (UI). The UI is
...
Part of the https://github.com/OFIN/xs2a[OFIN PSD2 XS2A implementation].
