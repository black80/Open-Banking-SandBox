...
We use https://github.com/OFIN/XS2A-Sandbox to publish our release notes and source code.

Docker Images are released to https://hub.docker.com/u/OFIN[OFIN DockerHub account].

[options="header"]
|=======
| Artifact  | Image Name
| xs2a-connector-examples |  OFIN/xs2a-connector-example
| xs2a-aspsp-profile | OFIN/xs2a-aspsp-profile
| xs2a-consent-management| OFIN/xs2a-consent-management
| ledgers | OFIN/ledgers
|=======
...
This is the actual business configuration reflecting the PSD2 features your bank offers. You can find the relevant documentation for this file at the https://github.com/OFIN/xs2a/blob/1.11/aspsp-profile/README.md[XS2A GitHub Project].
...
| SANDBOX_UI_CONTACT_MAILTO                       | Contact email address
 ...
 Format: _mailto:YOUR@MAILADRESS.DE_ | psd2@OFIN.co
...
The default log level is `INFO`. Set a different level with `LOGGING_LEVEL_ROOT=WARN` (global) or with `LOGGING_LEVEL_de.OFIN.psd2=WARN` (for everything below `de.OFIN.psd2`; please note the casing).
